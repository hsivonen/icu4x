...
fuzz_target!(|data: &[u8]| {
    ...
    if from_icu4x.len() > 180 {
        // rust_icu_unorm2 hard-codes the capacity to 200,
        // so we can't compare strings whose normalization
        // is longer than that. Making the max length shorter
        // to allow for variable length due to the kinds of
        // bugs this fuzzer is trying to find.
