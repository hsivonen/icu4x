...
fn compare_icu4c(
    ...
    collator.set_attribute(UColAttribute::UCOL_ALTERNATE_HANDLING, alternate_handling).unwrap();
    ...
    collator.set_attribute(UColAttribute::UCOL_CASE_FIRST, case_first).unwrap();
    ...
    collator.set_attribute(UColAttribute::UCOL_NORMALIZATION_MODE, UColAttributeValue::UCOL_ON).unwrap();

    collator.strcoll(&slice_to_icu4c(left), &slice_to_icu4c(right))
}
...
fuzz_target!(|data: &[u8]| {
    ...
    let option_bits = (u32::from(meta[1]) & 0b1111101101011) | 0b11111110000000000000000000000000;
