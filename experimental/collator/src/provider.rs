...
use crate::data_ce_to_primary;
use crate::CollationElement;
use crate::CollationElement32;
use crate::Tag;
...
pub mod key {
    ...
    pub const COLLATION_DATA_KEY: ResourceKey = resource_key!("collator/data@1");
    pub const COLLATION_DIACRITICS_KEY: ResourceKey = resource_key!("collator/dia@1");
    pub const COLLATION_JAMO_KEY: ResourceKey = resource_key!("collator/jamo@1");
    pub const COLLATION_METADATA_KEY: ResourceKey = resource_key!("collator/meta@1");
    pub const COLLATION_REORDERING_KEY: ResourceKey = resource_key!("collator/reord@1");
...
impl<'data> CollationDataV1<'data> {
    pub fn ce32_for_char(&self, c: char) -> CollationElement32 {
        CollationElement32::new(self.trie.get(c as u32))
    }
    pub fn get_ce32s(&'data self, index: usize, len: usize) -> &'data [RawBytesULE<4>] {
        &self.ce32s.as_ule_slice()[index..index + len]
    }
    pub fn get_ces(&'data self, index: usize, len: usize) -> &'data [RawBytesULE<8>] {
    ...
    pub fn get_default(&'data self, index: usize) -> CollationElement32 {
        let (ce32, _) = self.get_default_and_trie_impl(index);
        ce32
    }
    pub fn ce_from_offset_ce32(&self, c: char, ce32: CollationElement32) -> CollationElement {
    ...
    pub fn last_primary_for_group(&self, max_variable: MaxVariable) -> u32 {
...
impl<'data> CollationReorderingV1<'data> {
    pub fn reorder(&self, primary: u32) -> u32 {
        if let Some(b) = self.reorder_table.get((primary >> 24) as usize) {
            if b != 0 || primary <= super::NO_CE_PRIMARY {
